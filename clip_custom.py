...
class CLIP(nn.Module):
    def __init__(
        ...
        precomputed_text_embs_path=None
    ):
        super().__init__()
        ...
        if precomputed_text_embs_path is not None and not is_lora_text:
            print(f"Loading precomputed text embeddings from {precomputed_text_embs_path}")
            text_embs = torch.load(precomputed_text_embs_path, map_location="cpu")
            # đảm bảo chuẩn hóa
            text_embs = text_embs / text_embs.norm(dim=-1, keepdim=True)
            self.register_buffer("cached_text_feats", text_embs)
        else:
...
#         text_feats = einops.rearrange(text_feats, "(c p) d -> c p d", c=n_classes, p=n_prompts)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if hasattr(self, "cached_text_feats"):
          text_feats = self.cached_text_feats
        else:
        ...
        target_dtype = image_feats.dtype
        text_feats = text_feats.to(device=target_device, dtype=target_dtype)
